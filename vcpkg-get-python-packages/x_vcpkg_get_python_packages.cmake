...
function(x_vcpkg_get_python_packages)
        ...
        if(NOT EXISTS "${python_dir}/easy_install${VCPKG_HOST_EXECUTABLE_SUFFIX}")
            if(NOT EXISTS "${python_dir}/Scripts/pip${VCPKG_HOST_EXECUTABLE_SUFFIX}")
                if(arg_PYTHON_VERSION STREQUAL 3)
                    ...
                    vcpkg_execute_required_process(COMMAND "${arg_PYTHON_EXECUTABLE}" -E "${PYFILE_PATH}/public/get-pip.py" --no-warn-script-location
                ...
                elseif(arg_PYTHON_VERSION STREQUAL 2)
                    ...
                    vcpkg_execute_required_process(COMMAND "${arg_PYTHON_EXECUTABLE}" -E "${PYFILE}" --no-warn-script-location
                                                   WORKING_DIRECTORY "${CURRENT_BUILDTREES_DIR}"
                                                   LOGNAME "get-pip-${TARGET_TRIPLET}")
                endif()
            endif()
            vcpkg_execute_required_process(COMMAND "${arg_PYTHON_EXECUTABLE}" -E -m pip install virtualenv --no-warn-script-location
                                           WORKING_DIRECTORY "${CURRENT_BUILDTREES_DIR}"
    ...
    vcpkg_execute_required_process(COMMAND "${arg_PYTHON_EXECUTABLE}" -E -m "${python_venv}" ${python_venv_options} "${venv_path}"
                                   WORKING_DIRECTORY "${CURRENT_BUILDTREES_DIR}"
    ...
    if(DEFINED arg_REQUIREMENTS_FILE)
        ...
        vcpkg_execute_required_process(COMMAND "${${arg_OUT_PYTHON_VAR}}" -E -m pip install -r ${arg_REQUIREMENTS_FILE}
                                       WORKING_DIRECTORY "${CURRENT_BUILDTREES_DIR}"
    ...
    if(DEFINED arg_PACKAGES)
        ...
        vcpkg_execute_required_process(COMMAND "${${arg_OUT_PYTHON_VAR}}" -E -m pip install ${arg_PACKAGES}
                                       WORKING_DIRECTORY "${CURRENT_BUILDTREES_DIR}"
