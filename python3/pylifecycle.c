...
static wchar_t *dll_dirname;

int Init_vcpkg_DllSearchPath(void) {
  bool b = false;
  int res = 0;
  long long j = 0;
  VCPKG_PATH p_vcpkg_path;
  HMODULE dll_handle = NULL;
  
  if (GetModuleHandleExW((GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT),
                        (LPCSTR) &Init_vcpkg_DllSearchPath, &dll_handle) == 0)
  {
    // Getting the pythonxx.dll path failed
    //exe_filename =
  }
  int buffer_size = 1024;
  if(dll_dirname = (wchar_t*)malloc(buffer_size*sizeof(wchar_t))==NULL)
    return -1;
  if(GetModuleFileNameW(dll_handle, &dll_dirname[0], buffer_size - 1) == 0) {
    // Call Failed!
    return -1;
  }
  DWORD lerr = GetLastError();
  wchar_t *old_dll_dirname;
  while( lerr == ERROR_INSUFFICIENT_BUFFER)
  {
    buffer_size += buffer_size/2;
    old_dll_dirname = dll_dirname;
    if(dll_dirname = (wchar_t*)realloc(dll_dirname,buffer_size*sizeof(wchar_t))==NULL)
    {
      free(old_dll_dirname);
      return -1;
    }
    if(GetModuleFileNameW(dll_handle, &dll_dirname[0], buffer_size - 1) == 0) {
      // Call Failed!
...
#ifdef MS_WINDOWS
    auto err = Init_vcpkg_DllSearchPath();
    if(err != 0)
      return _PyStatus_ERR("Cannot initialize vcpkg dll search paths!");
#endif
